...
```{r class imbalance image, out.width = "300px", echo=FALSE, fig.align='center'}
...
```{r eval metrics validation, echo=FALSE, fig.align='center', out.width = "250px"}
...
The optimized model shows similar accuracy on the testing dataset. As seen below, our model achieves a testing accuracy score of 0.7397 overall. Like the validation metrics showed, the model predicts the `accepted` class better than it predicts the `rejected class` which is not ideal, but it also has a higher precision than recall, which, as discussed above, it preferable in this context.

**Model testing evaluation metrics:**
```{r eval metrics testing, echo=FALSE, fig.align='center', out.width = "250px"}
knitr::include_graphics("../results/clf_test_report.png")
```

The biggest predictor of rejection is the n-gram '88', though it's weight is very close in value to the weights in the top 20. listed below:

**Top 50 Predictors of Rejection:**
```{r best predictors, echo=FALSE, fig.align='center', out.width = "150px"}
